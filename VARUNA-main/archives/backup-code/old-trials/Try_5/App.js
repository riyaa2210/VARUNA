...
// --- CONFIGURATION ---
const CAMERA_URL = "http://10.125.48.116:8080/video"; 
...
function App() {
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (data.level > 0) {
                        ...
                        } else if (data.title === 'AMBULANCE DETECTED' && !hasRoutedRef.current) {
                            speakAlert("Ambulance Detected. Green Corridor Activated.");
                            hasRoutedRef.current = true; 
                        }
            ...
            ws.onclose = () => {
                if(!stopped) {
                    setConnectionStatus("Reconnecting ⚠️");
    ...
    const overlayClass = alert?.title === 'AMBULANCE DETECTED' ? 'detection-overlay ambulance' : (alert?.level > 1 ? 'detection-overlay critical' : 'detection-overlay');
        ...
        <div className="dashboard-container" style={{ backgroundColor: alert?.incident_active ? '#1a0000' : '#050505' }}>
            ...
            <div className="main-content">
                ...
                <div className="panel right-panel">
                            ...
                            <div style={{
                                ...
                                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                    <span style={{fontSize: '0.8rem', color: '#aaa'}}>
                                        🟢 {algorithmInfo.current_green_lane?.toUpperCase()}
                                    </span>
                                    <span style={{
                                        fontSize: '1.2rem', 
                                        fontWeight: 'bold', 
                    ...
                    {/* NEW: EMERGENCY PLAYBOOK DISPLAY */}
                    {alert?.level > 0 && (
                        <div style={{
                            margin: '10px',
                            padding: '12px',
                            ...
                            {alert.title === 'AMBULANCE DETECTED' && (
                                <div>
                                    <div style={{fontSize: '0.9rem', fontWeight: 'bold', color: '#00e5ff'}}>
                                        🚑 GREEN CORRIDOR (Algorithm #3)
                                    </div>
                                    <div style={{fontSize: '0.65rem', color: '#ccc', marginTop: '3px'}}>
                                        → All lanes RED except ambulance path<br/>
                                        → 60-second priority window<br/>
                                        → Override all normal traffic
                                    </div>
                                </div>
                            )}
