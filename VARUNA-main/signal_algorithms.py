...
class SmartSignalController:
    ...
    def decide_signals(self) -> SignalTimings:
        """
        Master function that selects the appropriate algorithm
        and returns the full SignalTimings object.
        
        Returns: SignalTimings(...)
        """
        # Detect incidents
        ...
        if fire_lanes:
            timing_decision = self.algorithm_fire_evacuation(fire_lanes[0])
        elif len(accident_lanes) >= 2:
            timing_decision = self.algorithm_multi_accident(accident_lanes)
        elif accident_lanes:
            timing_decision = self.algorithm_accident_diversion(accident_lanes[0])
        else:
            timing_decision = self.algorithm_normal_adaptive()
        
        # Update cycle tracking
        current_time = time.time()
        # Use the specific green time from the decision, or a default
        cycle_duration = timing_decision.timings.get(timing_decision.green_lane, self.BASE_CYCLE_TIME)
